    ...
    <div class="card-body p-0">
        ...
        @forelse ($tasks as $task)
                    <tr class="{{ $task->trashed() ? 'table-danger' : '' }}">
                        ...
                        <td>
                            <span class="badge {{ $task->priority === 'urgent' ? 'text-bg-danger' : ($task->priority === 'high' ? 'text-bg-warning' : ($task->priority === 'medium' ? 'text-bg-info' : 'text-bg-secondary')) }}">
                ...
                @if ($task->trashed())
                                <form action="{{ route('tasks.restore', $task->id) }}" method="POST" onsubmit="return confirm('Restore this deleted task?');">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="btn btn-success btn-sm">Restore</button>
                                </form>
                                <form action="{{ route('tasks.forceDelete', $task->id) }}" method="POST" onsubmit="return confirm('Permanently delete this task? This cannot be undone.');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-danger btn-sm">Delete</button>
                                </form>
                @else
                                <a href="{{ route('tasks.show', $task->id) }}" class="btn btn-outline-secondary btn-sm">View</a>
